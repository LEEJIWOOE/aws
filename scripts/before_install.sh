...
if ! command -v systemctl &> /dev/null || ! systemctl list-unit-files | grep -q tomcat; then
    ...
    # Amazon Linux 2023
    yum install -y tomcat || amazon-linux-extras install tomcat10 -y
fi
...
# 이전 배포 백업 (선택사항)
if [ -f /usr/share/tomcat/webapps/*.war ]; then
    echo "Backing up previous WAR file..."
    mkdir -p /home/ec2-user/backup
    cp /usr/share/tomcat/webapps/*.war /home/ec2-user/backup/app-backup-$(date +%Y%m%d-%H%M%S).war || true
fi
